...
let commentarios = ref([]);

const props = defineProps({
  isAuthenticated: Boolean,
  product: Object,
  user: Object,
  comments: Array, // Asegúrate de que estás pasando "comments"
});

onMounted(() => {
  if (props.comments) {
    commentarios.value = props.comments; // Asignamos los comentarios correctamente
  } else {
    console.warn("Los comentarios no están definidos en las props.");
  }

  console.log("Comentarios en ref:", commentarios.value);
  console.log("Comentarios en props:", props.comments);
});
  ...
  <component :is="isAuthenticated ? AuthenticatedLayout : NavbarS">
    <div class="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50 py-12">
      ...
      <div class="max-h-[300px] overflow-y-auto pr-4 space-y-6 custom-scrollbar">
              <div v-for="(comentario,i) in commentarios" :key="i" class="flex items-start space-x-4">
          ...
          <div class="flex-1">
            <div class="bg-white/80 backdrop-blur-sm rounded-lg p-4 shadow-sm hover:shadow-md transition-all duration-300">
              <div class="flex justify-between items-center mb-2">
                      ...
                      <span class="text-sm text-gray-500">{{ calcularTiempo(comentario.created_at) }}</span>
                    </div>
